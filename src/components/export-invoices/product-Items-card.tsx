import QRReaderPopup from "@/components/qr-reader-popup ";
...
interface ListItemExportProps {
  listItems: IItemWarehouse[];
  setListsNewItem: React.Dispatch<React.SetStateAction<IItemWarehouse[]>>;
  onRemoveItem?: (index: number) => void;
}

const ListItemExport = ({
  setListsNewItem,
  listItems,
  onRemoveItem,
}: ListItemExportProps) => {
      ...
      <Card className="h-full shadow-lg">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-lg justify-between">
            ...
            <QRReaderPopup setListsNewItem={setListsNewItem} />
          </CardTitle>
        </CardHeader>
        <CardContent>
