...
export const getColumns = (
  ...
  setSelectedExportInvoice: (exportInvoice: IExportInvoice) => void,
  setOpenSheet: React.Dispatch<React.SetStateAction<boolean>>
): ColumnDef<IExportInvoice>[] => [
  {
    accessorKey: "receiverName",
    header: "Tên người nhận",
    cell: ({ row }) => {
      const exportInvoice = row.original as IExportInvoice;
      return (
        <Button
          variant="link"
          className="p-0 text-left font-medium  hover:underline text-black dark:text-white"
          onClick={() => {
            setSelectedExportInvoice(exportInvoice);
            setOpenSheet(true);
          }}
        >
          {exportInvoice.receiverName}
        </Button>
      );
    },
    enableSorting: true,
