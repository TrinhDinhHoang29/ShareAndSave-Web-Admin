...
export const CreatePostInfoSchema = z
  .object({
    images: z
      .array(
        z
          ...
          .or(
            z
              .string()
              .startsWith("data:image/", { message: "Ảnh không hợp lệ" })
          )
      )
  ...
  .refine(
    (data) => {
      // Nếu cả hai đều có giá trị thì check
      if (data.startDate && data.endDate) {
        return data.startDate < data.endDate;
      }
      return true; // Không có đủ cả 2 giá trị thì hợp lệ
    },
    {
      message: "Ngày bắt đầu phải nhỏ hơn ngày kết thúc",
      path: ["endDate"], // báo lỗi ở endDate (hoặc startDate tuỳ ý bạn)
    }
  );
